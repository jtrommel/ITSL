...
With this function we generate 50 data points, adding a normally distributed error term $\espsilon ~ N(0,1).

<<>>=
train.df <- data.frame(x=seq(0, 100, by=2), y= 0)
sig1 <- 1
set.seed(2018)
eps.train <- rnorm(51, 0, sig1)
train.df$y <- 3 + 0.08*train.df$x + 0.0035*train.df$x^2 - 0.00004*train.df$x^3
train.df$p <- train.df$y + eps.train
test.df <- data.frame(x=seq(1, 99, by=2), y= 0)
sig2 <- sig1
eps.test <- rnorm(50, 0, sig2)
test.df$y <- 3 + 0.08*test.df$x + 0.0035*test.df$x^2 - 0.00004*test.df$x^3
test.df$p <- test.df$y + eps.test
ggplot() +
...
pred.lm.train <- predict(mod.lm)
pred.lm.test <- predict(mod.lm, newdata=new)
MSEtrain.lm <- mean((train.df$y - pred.lm.train)^2)
MSEtrain.lm
MSEtest.lm <- mean((test.df$y - pred.lm.test)^2)
MSEtest.lm
# checking the loess model
mod.loess <- loess(y ~ x, data=train.df, span=0.15)
pred.loess.train <- predict(mod.loess)
pred.loess.test <- predict(mod.loess, newdata=new)
MSEtrain.loess <- mean((train.df$y - pred.loess.train)^2)
MSEtrain.loess
MSEtest.loess <- mean((test.df$y - pred.loess.test)^2)
MSEtest.loess 
mod.loess <- loess(y ~ x, data=train.df, span=0.5)
pred.loess.train <- predict(mod.loess)
pred.loess.test <- predict(mod.loess, newdata=new)
MSEtrain.loess <- mean((train.df$y - pred.loess.train)^2)
MSEtrain.loess
...
where $y_{O}$ is the value of the dependent variable when $x=x_{0}$.
